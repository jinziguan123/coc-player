...
def test_migration_dedupes_shared_comfyui(monkeypatch, tmp_path):
    ...
    """
    monkeypatch.setattr(profile_store, "SETTINGS_FILE", _write(tmp_path, {"profiles": [
    ...
    images = profile_store._load_image_profiles()
...
def test_migration_inherits_text_base_and_key(monkeypatch, tmp_path):
    ...
    p = profile_store._load_image_profiles()[0]
...
def test_migration_is_idempotent_and_skips_unconfigured(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(profile_store, "SETTINGS_FILE", f)

    assert profile_store._load_image_profiles() == []
    ...
    assert len(profile_store._load_image_profiles()) == 1
    # 再读一次不会因为「文本配置没有生图字段」而把刚建的冲掉
    assert profile_store._load_image_profiles()[0].id == created["id"]
...
def test_saving_text_profiles_keeps_image_profiles(monkeypatch, tmp_path):
    ...
    assert len(profile_store._load_image_profiles()) == 1
    assert len(profile_store._load_profiles()) == 1
...
def test_image_profile_crud_and_activation(monkeypatch, tmp_path):
    ...
    assert profile_store.load_active_image_profile().name == "B"
    ...
    assert profile_store.load_active_image_profile().name == "A"

    c.delete(f"/api/settings/ai/image-profiles/{a['id']}")
    assert profile_store.load_active_image_profile() is None   # 一条不剩＝不出图，合法状态
...
def test_text_profile_no_longer_carries_image_fields(monkeypatch, tmp_path):
    ...
    assert profile_store._load_image_profiles() == []   # 没有从文本配置里凭空冒出生图配置
