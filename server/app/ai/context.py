...
# 被动注入的规则书要点：单块截断上限。切块本体 500 字、检索侧相邻块合并后最长约
# 920 字（500+420）——上限必须 ≥ 合并块长，否则检索到的条文尾部被注入环节砍掉
# （此前 400 < 500 就砍掉过每块末尾约 100 字）。典型轮次仍是 1-3 块 × ≤500 字。
RULE_EXCERPT_MAX_CHARS = 1000
...
def _improvised_npc_section(session: GameSession) -> str:
    ...
    # 同时滤掉台词归属误命中的垃圾名与只出现过一次的切割残片，别把旁白碎片当龙套注入 prompt。
    names = [
        str(n).strip() for n, e in improv.items()
        if str(n).strip()
        and not (isinstance(e, dict) and (e.get("card") or {}).get("id"))
        and world_memory.improvised_confirmed(str(n), e)
    ]
    if not names:
        return ""
    return (
...
def _collect_kp_facts(
    ...
    if teammates:
        ...
        player_info = (
            ...
            + roster
            + "\n\n**多人叙事铁律（违反即严重错误）**：\n"
            "1. **人称——用姓名指代，绝不用「你」称呼单个玩家角色**：本场有多名玩家角色，叙述**必须以"
            "完整姓名（第三人称）**分别指代每一位（如「詹姆斯俯身拉开抽屉」「哈桑竖起耳朵分辨声响」），"
            "**绝对不要用「你」**去指某一个玩家角色——多人局里「你」指代不明，玩家分不清在说谁，这是最常犯的错。"
            "只有面向**全体**的环境/氛围才可用「你们」（如「你们被困在墓中」）。"
            "（本提示别处出现的「你察觉/你意识到」等是**单人写法示例**，多人局一律换成对应角色的姓名。）\n"
            "2. **平等对待所有玩家角色**——没有「主角」一说（名册第一位只是房主、并不更重要）；"
            "戏份要**跨回合**照顾到所有人，但**本轮只叙述实际行动者**（见第 4 条），不靠替人行动来均衡。\n"
            "3. **绝不替任何玩家角色行动或说话**——不写他们的台词、不描述他们的主动行动/姿态/"
...
def _assemble_kp_messages(
    ...
    if not events:
        ...
        # 这条提醒只该说一件事：**别把台词省掉**。它一度还规定「用中文双引号包裹写进正文」
        # ——那是 say() 工具出现之前的写法，两边并存之后就成了每轮贴在玩家输入前的反向指令：
        # 工具要求台词走 say()，它要求台词写进旁白。模型照着写一次，那条旁白就作为 assistant
        # 消息回灌进下一轮，自我模仿把违规率越滚越高（闇暗山存档实测：前期 say 17:漏 8，
        # 后期反转成 6:11）。故只保留「不许省略台词」，出口一律指回 say()/[SAY]。
        if len(messages) >= 3 and messages[-1]["role"] == "user":
            messages.insert(-1, {
                "role": "system",
                "content": (
                    "[格式提醒] NPC 开口就要给出完整台词，"
                    "不要只描述其声音、语气或动作而省略台词内容。"
                    "台词一律走对话出口（say 工具，或未启用工具时的 [SAY] 标记），"
                    "**不要把台词写进叙述文本**——写进正文的引号不会成为对话气泡。"
                ),
            })
