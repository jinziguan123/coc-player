...
REGISTRY: tuple[ToolSpec, ...] = (
    ToolSpec(
        ...
        tag="DICE_CHECK",
        description=(
            "发起一次**普通技能/属性**检定。只在结果不确定、且成败都有意义时使用；必然成功或无关紧要"
            "的事直接叙述。调用前只描写角色「正在尝试」的过程动作，绝不预测、暗示或写出检定"
            "结果（不写「找到/发现/听到了什么」）。玩家表达观察/搜索/聆听/调查/辨认/回忆知识等"
            "感知或求知行动时必须先发起对应检定，结果回注后再按达成等级分层给信息。"
            "\n\n**下列情形有专用工具，一律不要用本工具凑**（专用工具会连带处理损失、"
            "胜负判定、状态机切换等本工具做不到的结算）："
            "\n- 理智 / SAN → san_check（本工具无法扣 SAN、不会判疯狂）"
            "\n- 双方比拼胜负（潜行 vs 侦查、话术 vs 意志、擒抱等）→ opposed_check"
            "\n- 进入你死我活的交战 → start_combat；有人逃跑/被追 → start_chase"
            "\n- 变更所在场景 → scene_change；查规则细则 → rule_lookup；查模组原文 → module_lookup"
        ),
        parameters=_params({
            "skill": {
                "type": "string",
                "description": (
                    ...
                    "系统自动按属性值判定。"
                    "**不要填「理智」/「SAN」**——理智检定请改用 san_check 工具。"
                ),
            },
            "difficulty": {
                "type": "string",
            ...
            "modifier_reason": {
                "type": "string",
                "description": (
                    "**填了 bonus 或 penalty 就必须填**：一句话说明凭什么加这个奖励/惩罚骰"
                    "（如「手电筒照明充足」「肩上的伤在流血」「桌子挡住了大半视野」）。"
                    "系统会把它连同骰子一起摆给玩家看——奖惩骰改的是成败概率，"
    ...
    ToolSpec(
        ...
        description=(
            ...
            "血腥惨状 1/1d6，遇怪物 1/1d6，强大神话生物 1d6/1d20。真人玩家亲自投骰，"
            "AI 角色由系统自动掷；结果出来前勿预测损失。"
    ...
    ToolSpec(
        ...
        description=(
            ...
            "③ 不在场的人不能说话；④ 台词只能出自**这个说话人自己知道的事**——场景列表、线索、"
            "幕后真相、其他 NPC 的设定都是你的作者底稿，不是他的记忆，他知道什么由其身份与经历决定。"
            "同一个 NPC 前后说法必须自洽：已经让他说过「不知道」的事，之后不能突然知道；"
            "反过来，他按身份本就该知道的事，第一次被问到时就直说，不要先卖关子再被追问着改口。"
        ),
        parameters=_params({
