...
CoC Player 是一个本地优先的 AI 跑团桌面应用：AI 担任主持人和 NPC，玩家可独自开团，也可在可信局域网内邀请其他玩家加入。

**规则系统只实现了《克苏鲁的呼唤》第七版**。规则相关逻辑走插件式 `RuleEngine`，接别的规则系统在架构上是通的，但目前只有 CoC 一套引擎——项目因此叫 CoC Player 而不是泛化的名字。
