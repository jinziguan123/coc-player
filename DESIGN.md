...
- [六、回合主链路](#六回合主链路)（含 [6.1 四个阶段与模型分派](#61-四个阶段两条纪律线)）
- [七、实时层：房间事件与重连](#七实时层房间事件与重连)
- [八、规则引擎与 AI 的边界](#八规则引擎与-ai-的边界)
- [九、记忆、检索与上下文预算](#九记忆检索与上下文预算)（含 [9.1 一次请求的注入规则](#91-一次请求的注入规则)）
- [十、联机与信任边界](#十联机与信任边界)
- [十一、桌面分发链路](#十一桌面分发链路)
...
这也是为什么 `run_chat_generation` 开头要**等上一轮的后台任务写完**——它们要动 `world_state`，
玩家手快时会替上一轮的后台工作买单，且此前完全看不见（现已单独计时）。

**模型分派的判据是「产出给谁看」**，不是「任务重不重」：

| 环节 | 走哪个模型 | 判据 |
|---|---|---|
| TurnPlan、TurnValidator、摘要、幕后 | 快模型 | 产出是结构，玩家看不到；低温、要的是稳定判断而非文笔 |
| KP 叙事、NPC 台词、**AI 队友** | 主模型 | 产出直接摆在玩家面前，和 KP 叙事一样吃文笔与分寸感 |

队友走主模型是一处容易搞反的地方：它看着像「副任务」，但快模型（通常关思考、档次也更低）
演出来明显发木，同桌感立刻塌掉。慢一点也认。

**planner 为什么跑两次。** `TurnPlan` 先在 AI 队友回合**之前**跑一次，作为本回合的共享
契约——队友据 `plan.direction` 派生的导演提示行动（例如把话头递给被冷落的玩家）。但队友
行动之后前提可能真的变了（他挪了位置、开了箱子、掷了骰、触发了机制点），回合起点那份
plan 就不能继续作为最终副作用裁定，需要重规划。

判据是 `_team_turn_changed_premises`，**刻意从宽认定「变了」**——除对白外的任何新事件都算。
因为判错的代价不对称：漏判会让 KP 拿着过时的裁定叙事，多判只是慢几十秒。反过来，队友
只是接了句话时跳过重规划是实打实的省时：实测那一次要 46 秒，占整个回合的 44%
（两个队友分别说了「背面也写了字？让我看看」和「便签背面写了什么？」，planner 于是对着
一模一样的前提把「扯下便签」重判了一遍）。

**一条图里没画的旁路**：玩家明确申请检定时（planner 的 `player_check_request` 命中，或技能页
点出来的 `check_request` 元数据），直接走确定性检定裁定，**跳过常规叙事**——否则申请会被
KP 当成叙事顺过去，玩家点的那一下等于没发生。明确的战斗宣言不走此路。

**分头行动**：分头与否是**确定性状态**——按 `world_state.party_locations` 归并
（`_location_groups`），身处 ≥2 个场景才算分头，不看角色嘴上说要不要分头。分头时逐场景生成，
**各列共用同一份 `TurnPlan`**（即进入本次生成的那一份，可能是二次规划的结果），
...
- **产物不得随分词漂移。** 线上 Provider 怎么切 token 不可控，因此 `result[0..4]`
  （旁白、`full_response`、气泡、对话位点、分组）必须与切分方式无关，由
  `tests/test_narration_protocol_golden.py` 按 1/2/3/5/13 字与整串逐一比对钉住。
  `full_response` 尤其要紧：它是下游 `_process_commands` 解析指令的输入，
  而后者对 `SET_FLAG` / `BLOCK_PATH` 这些用的是 `finditer`。曾经它按 token 整块累加
  （`+= token`），而终止标签处的 `break` 只跳字符循环，于是模型写
  `[SET_FLAG: a][SET_FLAG: b]` 时第二条**时而执行、时而不执行**——全看那两个标签有没有
  落在同一个 token 里。现按扫描器实际消费的字符切片累加，语义以
  **「第一个终止标签就停」**为准（叙事本来也在那里截断），第二条一律不执行。
...
所以新增小节时第一个要回答的问题是「它属于哪一档」，而不是「放哪儿读着顺」。
三档拼成一条 system 消息，分块结果另挂 `CACHE_BLOCKS_KEY` 供支持缓存的 Provider 打断点。

**超预算时按 `priority` 从大到小整段丢弃**，不从尾部切字符——切字符会把模组 JSON 切碎，
模型读到半截结构比读不到更糟。判据是**丢了能不能捞回来**：

| priority | 小节 | 为什么排这儿 |
|---|---|---|
| 40（先丢） | 模组摘录、规则摘录 | KP 手上有 `[MODULE_LOOKUP]`/`[RULE_LOOKUP]`，真需要会自己去拿——可恢复 |
| 35 → 10 | 临场角色、幕后动态、手书、封路、NPC 记忆 | 影响真实但不致命，丢了 KP 仍能把这一轮演完 |
| 6 → 2 | 记账指令、线索台账、场景机制点、队伍位置 | 丢了玩家**直接可感**：已发现的线索被重演一次发现桥段、一次性桥段被照着 JSON 重演、KP 又开始猜分头 |
| 0（永不丢） | 身份、裁定手册、模组数据、战斗态、战斗余波 | 丢了状态机就悬空。宁可超预算，代价由事件区吸收（历史少给几条） |

**一处反直觉的地方**：`_estimate_tokens` 的启发式（中文 1.5 token/字）系统性高估真实 BPE
（中文约 1 token/字）近 50%。固定系数的好处是有安全垫，代价是**永远只用到窗口的约 40%**——
而逐字记忆恰恰是对抗有损摘要的唯一资源，省在这里最不划算。`budget_scale` 就是拿真实
usage 反过来校准估算口径，把这部分省错的窗口还回来。
...
- **触发**：`_run_generation` 中，**仅非开场**（`events` 非空）才跑；开场或规划失败则不注入，
  KP 走原逻辑。主链路把它前移到了队友回合之前（见 [§6.1](#61-四个阶段两条纪律线)），
  队友改变裁定前提时会再跑一次；**进入本次生成的那一份计划由分头/单场景各列共用**。
...
队伍身处 ≥2 个场景时逐场景生成（`_run_split_generation`）：**列数不影响 planner 的次数**——
进入本次生成的那一份计划注入每一列（分几组就是几组，planner 不会跟着跑几遍）；
每列**各自**过一遍 validator。另外每列以**自身所在场景**为锚构建 KP 上下文
