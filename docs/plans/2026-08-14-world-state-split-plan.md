...
> 进度（2026-08-14）：**已全部落地并逐项提交**，后端 pytest 1676 通过 + ruff 通过。
> 具体：Phase 1（combat → combat_states）、Phase 2（chase → chase_states）、
> Phase 3（usage/rag → session_stats）、Phase 4a（recaps → session_recaps）、
> Phase 4b（回合锁 turn_confirm → turn_state 列）、Phase 4c（剧情记忆 Pydantic
> schema + SCHEMA_VERSION 1→2 + fail-open 校验）、Phase 5（pending_checks /
> pending_item_gains / item_delta_keys → turn_state）、Phase 6（party_locations /
> visited_scenes → session_navigation；san_checked / scene_events_seen → session_ledger）。
> 至此 world_state 只承载剧情记忆 + 刻意留下的 pending_clue_reveals（与 clue_ledger
> 同生同灭）等流程键。
